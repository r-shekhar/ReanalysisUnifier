...
def calc_derived(reanal, dsets, data):
    ...
    pressure_levels = dsets['T'].variables['level'][:]
    # check if units are in hPa. Metadata isn't always trustworthy
    if np.max(pressure_levels < 2000.):
    ...
    fh = clone_netcdf_skeleton(reanal, dsets, "GHT",
                               ...
                               "J kg-1")
    apply_linear_formula(fh['MSE_SATURATED'],
                         formula=((atmos_thermo.catmos.C_pd, data['T']),
                                  (atmos_thermo.catmos.Lv0, rsat),
                                  (g_effective, data['GHT']),
                                  )
                         )
    fh.close()
